...
func NewSleepRouteService(conn *grpc.ClientConn) *SleepRouteService {

	client := sleep.NewSleepServiceClient(conn)
	return &SleepRouteService{
		client: client,
	}
}

type SleepRouteService struct {
	client sleep.SleepServiceClient
}

func (s *SleepRouteService) Sleep(ctx context.Context, req *sleep.SleepRequest) (*sleep.SleepResponce, error) {
